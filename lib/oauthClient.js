class OAuthClient {
    ...
    async loadAxios() {
      ...
      startAuthFlow() {
        // ! defined scope for offline inorder to get refresh token
        const url = `${this.authUrl}?response_type=code&client_id=${this.clientId}&redirect_uri=${encodeURIComponent(this.redirectUri)}&scope=openid offline_access`; 
        if (typeof window !== 'undefined') {
    ...
    async handleCallback(code) {
        ...
        try {
          ...
          console.log('Token response:', response.data); // Add this line for debugging
